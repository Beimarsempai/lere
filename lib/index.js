...
const ownerNumber = "+59160703694"
const ownerName = "Beimar"
...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:Italu🧙‍♂️\n' 
            + 'ORG:Dono do Tiringa;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=59160703694:59160703694\n' 
            + 'END:VCARD'

//_TIC-TAC-TOE By: Resen
const { ttthelp } = require('./database/ttt/TTTconfig/ttthelp.js');
const { tttme } = require('./database/ttt/TTTconfig/tttme.js');
...
tiringa.on('group-participants-update', async (anu) => {
      ...
      tiringa.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = [`59160703694@s.whatsapp.net`]
			const ownerNumberB = []
			//const ban = [`59160703694@s.whatsapp.net`]
...
(budy === 'BOT') ||
(budy === 'Bot')) {
buf = fs.readFileSync(`./mp3/lala17.mp3`)
...
//INICIO DO JOGO DA VELHA ❌ ⭕ 🔲
				case 'ttt':
					...
					if (!isGroup) {
						...
						reply(`Alguien ya esta jugando\nPor favor aguarde un instante...`)
					} else if (tttset.waitingTime == "on") {
						reply(`Alguien jugo recentemente\nPor favor aguarde un tiempo de espera...`)
					} else if (args == 0 || (args != 'easy' && args != 'Easy' && args != 'EASY' && args != 'normal' && args != 'Normal' && args != 'NORMAL' && args != 'hard' && args != 'Hard' && args != 'HARD'&& args != 'impossible'&& args != 'Impossible' && args != 'IMPOSSIBLE')) {
						reply(`Defina a dificuldade\nEx.: ${prefix}ttt easy\n\nDificuldades: easy, normal, hard e impossible`)
					} else if (limitrl !== undefined && cdd - (Date.now() - limitrl) > 0) {
						reply('Para jugar nuevamente intente en 8m.')
					} else {
						tttset.tttstatus = "on"
